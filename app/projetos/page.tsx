            ...
            {PROJECTS.map((project, i) => {
              ...
              return (
                <div
                  ...
                  aria-label={project?.title ?? p.comingSoon}
                  onClick={() => { if (!isDragging.current) go(i); }}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); if (!isDragging.current) go(i); } }}
                  style={{
                    position: "absolute",
                    width: cardW,
                    height: cardH,
                  ...
                  {project?.soon ? (
                    <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
                      ...
                      {!project.image && QUESTION_MARKS.map((q, qi) => (
                        <span key={qi} aria-hidden="true" className={`qm-${qi}`} style={{
                          position: "absolute",
